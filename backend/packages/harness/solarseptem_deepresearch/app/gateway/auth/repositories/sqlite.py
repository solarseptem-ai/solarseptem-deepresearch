...
``solarseptem_deepresearch.persistence.engine`` — the ``users`` table lives in the
same database as ``threads_meta``, ``runs``, ``run_events``, and
``feedback``.

Constructor takes the session factory directly (same pattern as the
other four repositories in ``solarseptem_deepresearch.persistence.*``). Callers
...
from solarseptem_deepresearch.app.gateway.auth.models import User
from solarseptem_deepresearch.app.gateway.auth.repositories.base import UserNotFoundError, UserRepository
from solarseptem_deepresearch.persistence.user.model import UserRow
