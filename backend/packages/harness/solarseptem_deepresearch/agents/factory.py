...
from solarseptem_deepresearch.agents.features import RuntimeFeatures
from solarseptem_deepresearch.agents.thread_state import ThreadState
from solarseptem_deepresearch.agents.tools.builtins import ask_clarification_tool
from solarseptem_deepresearch.agents.middlewares.clarification_middleware import ClarificationMiddleware
from solarseptem_deepresearch.agents.middlewares.dangling_tool_call_middleware import DanglingToolCallMiddleware
from solarseptem_deepresearch.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
...
def create_solarseptem_agent(
    ...
    *,
    ...
    """Create a Solarseptem agent from plain Python arguments.
    ...
    if extra_middleware:
        for emw in extra_middleware:
            if not isinstance(emw, AgentMiddleware):
                raise TypeError(f"extra_middleware items must be AgentMiddleware instances, got {type(emw).__name__}")
...
def _assemble_from_features(
    ...
    *,
    ...
    if plan_mode:
        from solarseptem_deepresearch.agents.middlewares.todo_middleware import TodoMiddleware
        from solarseptem_deepresearch.prompts.system import _TODO_SYSTEM_PROMPT,_TODO_TOOL_DESCRIPTION
    ...
    if feat.auto_title is not False:
        if isinstance(feat.auto_title, AgentMiddleware):
            ...
            from solarseptem_deepresearch.agents.middlewares.title_middleware import TitleMiddleware
    ...
    if feat.memory is not False:
        if isinstance(feat.memory, AgentMiddleware):
            ...
            from solarseptem_deepresearch.agents.middlewares.memory_middleware import MemoryMiddleware
    ...
    if feat.vision is not False:
        if isinstance(feat.vision, AgentMiddleware):
            ...
            from solarseptem_deepresearch.agents.middlewares.view_image_middleware import ViewImageMiddleware

            chain.append(ViewImageMiddleware())
        from solarseptem_deepresearch.agents.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        if isinstance(feat.subagent, AgentMiddleware):
            ...
            from solarseptem_deepresearch.agents.middlewares.subagent_limit_middleware import SubagentLimitMiddleware

            chain.append(SubagentLimitMiddleware())
        from solarseptem_deepresearch.agents.tools.builtins import task_tool

        extra_tools.append(task_tool)

    # --- [12] LoopDetection (always) ---
    from solarseptem_deepresearch.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
