...
class MemoryConfig(BaseModel):
    ...
    storage_path: str = Field(
        ...
        description=(
            ...
            "Note: if you previously set this to `.solarseptem-dr/memory.json`, "
            "the file will now be resolved as `{base_dir}/.solarseptem-dr/memory.json`; "
            "migrate existing data or use an absolute path to preserve the old location."
        ),
    )
    storage_class: str = Field(
        default="solarseptem_deepresearch.agents.memory.storage.FileMemoryStorage",
