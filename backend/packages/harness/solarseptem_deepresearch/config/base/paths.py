...
class Paths:
    ...
    def host_base_dir(self) -> Path:
        ...
        Set SOLARSEPTEM_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("SOLARSEPTEM_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("SOLARSEPTEM_HOME"):
            return Path(env_home).resolve()

        cwd = Path.cwd()
        if cwd.name == "backend" or (cwd / "pyproject.toml").exists():
            return cwd / ".solarseptem-flow"

        return Path.home() / ".solarseptem-flow"
